...
### Диалоги
- `POST /api/businesses/:businessId/dialogs/start` - Начать диалог с бизнесом (требует авторизации)
- `GET /api/dialogs/me` - Получить мои диалоги (требует авторизации, для пользователя)
- `GET /api/businesses/:businessId/dialogs` - Получить диалоги бизнеса (требует авторизации, только владелец)
- `GET /api/dialogs/:dialogId` - Получить диалог (требует авторизации, участник)
- `GET /api/dialogs/:dialogId/messages` - Получить сообщения диалога (требует авторизации, участник)
- `POST /api/dialogs/:dialogId/messages` - Отправить сообщение (требует авторизации, участник)
- `PATCH /api/dialogs/:dialogId/read` - Отметить как прочитанный (требует авторизации, участник)

**Требования к UI:**
- Для пользователя:
  - Кнопка "Начать диалог" на странице бизнеса
  - Страница диалога (`/business/:slug/dialog`) - интерфейс одного диалога
  - Отображение сообщений в стиле мессенджера (пузыри)
  - Поле ввода и отправка сообщений
  - Автоскролл к новым сообщениям
  - Пагинация при скролле вверх
- Для владельца бизнеса:
  - Страница списка диалогов (`/my-businesses/:businessId/dialogs`)
  - Список всех диалогов с пользователями
  - Отображение: имя пользователя, последнее сообщение, время, бейдж непрочитанных
  - Сортировка по последнему сообщению (новые первыми)
  - Поиск по имени пользователя
  - Открытие диалога по клику
  - Бейдж с непрочитанными на кнопке "Диалоги"
